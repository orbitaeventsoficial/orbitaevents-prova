...
export default function DiscomovilClientV2() {
  ...
  const generateWhatsAppMessage = () => {
    ...
    let message = `🎉 ¡Hola! Quiero presupuesto para mi fiesta\n\n`;
    message += `📦 Pack: ${config.selectedPack.name} (${config.selectedPack.priceValue}€)\n`;
    message += `👥 Personas: ${config.numGuests}\n`;
    
    if (config.extraHours > 0) {
      message += `⏰ Horas extra: +${config.extraHours}h (+${extraHoursPrice}€)\n`;
    }
    ...
    if (hasComboDiscount) {
      message += `🎁 Descuento Combo: -${discount}€\n\n`;
    }
